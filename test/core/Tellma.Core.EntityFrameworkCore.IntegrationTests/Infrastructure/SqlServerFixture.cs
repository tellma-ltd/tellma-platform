...
[assembly: AssemblyFixture(typeof(Tellma.Core.EntityFrameworkCore.IntegrationTests.Infrastructure.SqlServerFixture))]

namespace Tellma.Core.EntityFrameworkCore.IntegrationTests.Infrastructure
{
    /// <summary>
    ///     One SQL Server for the whole integration assembly. By default a Testcontainers-managed
    ///     container (identical locally and in CI); set <c>TELLMA_TEST_SQL</c> to a connection
    ///     string to target LocalDB or a local SQL Server for faster inner-loop runs. Each test
    ///     gets its own database via <see cref="CreateDatabaseAsync" />.
    /// </summary>
