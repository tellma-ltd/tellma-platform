...
/**
 * The synchronous outcome of a label commit that awaits a column resolver:
 * the addressed cell (already written to its cleared value) and the OPEN
 * history entry the eventual resolution writes into — one undo entry covers
 * the commit and its async outcome together.
 */
export interface TmGridPendingLabelCommit {
  /** The committed cell's row identity (materialized for a placeholder session). */
  readonly rowId: TmRowId;
  /** The committed column's id. */
  readonly columnId: string;
  /** The committed column's model key. */
  readonly columnKey: string;
  /** The open history entry; the resolution finalizes it. */
...
export class TmGridEditState<T = unknown> {
  ...
  /**
   * Commits editor text through the label-resolution ladder — the same
   * conversion chain a paste runs, applied to one editor commit: empty text
   * writes the cleared value; a successful consumer parse commits
   * synchronously; unparseable text on a column WITH a resolver writes the
   * cleared value inside an OPEN history entry and returns the pending
   * commit for the caller to run the resolver against (the eventual
   * resolution lands in the same entry — one undo step); without a
  ...
  commitLabel(text: string): TmGridPendingLabelCommit | null {
    ...
    const model = this.options.model;
    const column = model.columnAt(session.cell.col);
    if (column === undefined || column.key === null) {
      this.sessionSignal.set(null);
      return null;
    }
    // (1) Empty text clears the cell (the paste ladder's empty rung — it
    // never reaches parse or the resolver).
    if (text === '') {
      this.commitInternal((rowId, cell) => this.buildWrite(rowId, cell, column.clearedValue, null));
      return null;
    }
    // (2) A successful consumer parse commits synchronously (with the
    // display-scale normalization commitText applies).
    if (column.parse !== undefined) {
      const parsed = column.parse(text, { locale: untracked(() => this.options.locale()) });
      if (parsed !== TM_PARSE_ERROR) {
    ...
    // (4) The resolver rung: clear the cell inside an OPEN entry and hand
    // the caller the addresses; a placeholder session materializes its row
    // into the same entry first.
    this.sessionSignal.set(null);
    const handle = this.options.history.beginCompound('cellEdit');
    let rowId = session.rowId;
    if (rowId === null) {
      ...
      rowId = created[0].id;
    } else if (model.modelIndexOfRow(rowId) === -1) {
      handle.finalize();
      return null; // the row vanished mid-edit; reconcile already announced
    }
    const write = this.buildWrite(rowId, session.cell, column.clearedValue, null);
    if (write !== null) {
      handle.applyWrites([write]);
    }
    this.options.onEdit?.();
    return { rowId, columnId: column.id, columnKey: column.key, handle };
  }
