...
function sharedVars(tokens: TmTokens): VarMap {
  ...
  // Adaptive UI aliases: components consume these; the [dir='rtl'] block
  // below flips them to the Arabic family + leading (brand typography rule
  // expressed at the token tier, so it survives any component-level
  // font-family declaration).
  vars.set('--font-ui', 'var(--font-sans)');
...
export function tmEmitCss(tokens: TmTokens): string {
  ...
  const rtlVars: VarMap = new Map([
    ['--font-ui', 'var(--font-arabic)'],
    ['--leading-ui', 'var(--leading-arabic)'],
  ]);
  ...
  return [
    ...
    '',
    "  /* Arabic type on RTL subtrees (§7): the adaptive aliases flip here. */",
    block("[dir='rtl']", rtlVars, '  '),
    '}',
    '',
  ].join('\n');
}
