 ...
 *
 * - `--tm-*` are component-PRIVATE locals, declared in the same CSS that
 *   reads them (the `--tm-` prefix is reserved for exactly this).
...
export function tmCheckCssRefs(emittedCss: string, files: readonly string[]): string[] {
  ...
  for (const file of files) {
    ...
    for (const ref of tmVarRefs(css, file)) {
      ...
      seen.add(key);
      problems.push(`${basename(file)} reads ${ref.name}, which no token emits`);
    }
  }
  return problems;
}
