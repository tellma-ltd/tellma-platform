 ...
 * runtime axes stay addable by swapping token sets alone. Allowed literals:
 * 0, entries in the `allow` option (hairline "1px" by default at config
 * level), and non-sizing properties (e.g. the `border` shorthand's 1px).
 */
...
const SIZING_PROPS =
  /^(height|min-height|max-height|width|min-width|max-width|inline-size|min-inline-size|max-inline-size|block-size|min-block-size|max-block-size|padding|padding-[a-z-]+|margin|margin-[a-z-]+|gap|row-gap|column-gap|font-size|line-height|border-radius|border-(start|end)-(start|end)-radius|inset|inset-[a-z-]+|top|right|bottom|left)$/;
