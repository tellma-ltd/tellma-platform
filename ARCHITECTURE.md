...
| Package | Role |
|---|---|
| `@tellma/core-ui-primitives` | Headless behavior layer. Each component's state, keyboard, selection, and validation logic lives in a `Tm*Pattern` class that is not an Angular component/directive/service (no template, DI, or lifecycle): inputs typed `SignalLike`, reactive state via the `@angular/core` **signal primitives only** (`signal`/`computed`/`linkedSignal` — never the component/DI/template layer, no `effect()` since there is no injection context), built on `@angular/aria` behaviors where they exist. A symbol-level import-boundary lint enforces the allowlist. |
