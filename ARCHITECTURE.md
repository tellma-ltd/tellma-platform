...
| **`@tellma/core-ui` family** | The Angular UI component library, shipped as Core-layer npm packages (`@tellma/core-ui`, `-primitives`, `-tokens`, `-testing`, `-mcp`). Greenfield on `@angular/cdk` + `@angular/aria`; `tm-`-prefixed; signal-first. See [Frontend → UI component library](#ui-component-library). |
...
│   │   │   ├── tellma-core-ui-primitives/         # @tellma/core-ui-primitives — headless Tm*Pattern + behaviors (on @angular/cdk + @angular/aria)
│   │   │   ├── tellma-core-ui/                    # @tellma/core-ui — styled tm-* components
...
| User-defined table types (UDTTs) | Generated by EF Core migrations from the same entity classes, for tables that opt in | The `Tellma.Core.EntityFrameworkCore` extension derives each opted-in table's UDTT as a row image of the table (writable columns minus exclusions, PK mirrored) and emits it through first-class `CreateTableType`/`DropTableType` migration operations. No index/ordinal columns — IDs are app-assigned before save (see [ID allocation](#id-allocation--app-assigned-from-sequences)). |
...
**Schema-on-fork: the schema is part of the entity's declaration.** A fork replaces the implementation, never the canonical table name. `[gl].Invoices` and its UDTT `[gl].[InvoicesList]` keep those names regardless of which distro deploys them. External consumers (SSMS queries, BI dashboards, monitoring) see consistent canonical names everywhere.
...
2. **Derived row image.** The UDTT's columns, store types, facets, nullability, collation, and order are taken from the relational model EF already built for the table — included columns are the insertable/updatable columns minus exclusions, computed columns always excluded; the PK mirrors the table's PK; rowversion columns are included as nullable `binary(8)` by default (excludable); never IDENTITY, defaults, FK or named constraints. **Column order is part of the type's contract** (TVP binding is ordinal) and is captured in the model snapshot so a pure reorder produces a diff.
3. **First-class migration operations.** The differ emits `CreateTableTypeOperation`/`DropTableTypeOperation` (with `MigrationBuilder.CreateTableType`/`DropTableType` extensions for manual authoring), and the migrations SQL generator handles them — including `dotnet ef migrations script --idempotent` and bundles. SQL Server has no `ALTER TYPE`, so every definitional change scaffolds a Drop + Create pair in the same migration; drop SQL is preceded by a `sys.sql_expression_dependencies` pre-flight that THROWs with the dependent modules by name; configured `GRANT EXECUTE ON TYPE` statements are re-emitted after every (re)create.
...
Deployed UDTT names default to the table's own schema plus a `List` suffix — `[gl].[InvoicesList]`, `[gl].[InvoiceLinesList]` — overridable per entity. Column types match the corresponding table columns byte-for-byte. The library also ships a small set of hand-defined primitive types outside the 0-or-1 rule for bulk delete / bulk lookup: `[IdList]` (`int`), `[BigIdList]` (`bigint`), `[GuidList]` (`uniqueidentifier`), `[StringList]` (`nvarchar(450)`).
...
Migrations are applied by a dedicated console project per distribution — `Tellma.Distro.<Slug>.Migrator` — run once per deployment from CI/CD (a pipeline step before the slot swap), never per-instance at web-app startup. The migrator references the Web project, so the same `AddTellma(…)` composition that serves traffic builds the model that generates and applies migrations; it is also the `dotnet ef` design-time target and owns the `Migrations/` folder. This is what keeps Design-time dependencies (Roslyn, templating) out of the web server's publish output entirely.
...
- **New tenants are born converged.** Tenant provisioning applies the full migration chain + seeds at creation time.
...
The reusable UI ships as a `core-*` package family in the Angular workspace under `client/projects/core/`, part of the bare-minimum Core layer every distribution references and family-major versioned with the rest of the platform. It is built greenfield on `@angular/cdk` + `@angular/aria` — never by extending Material or PrimeNG, and never through a framework-agnostic styling engine (Angular-only, permanently). The Material/PrimeNG comparison and the rationale behind every choice below live in [`docs/research/angular-component-library-analysis.md`](docs/research/angular-component-library-analysis.md).

| Package | Role |
|---|---|
| `@tellma/core-ui-primitives` | Headless behavior layer. Each component's state, keyboard, selection, and validation logic lives in a framework-decoupled `Tm*Pattern` class (inputs typed `SignalLike`, no `@angular/core` dependency), built on `@angular/aria` behaviors where they exist. |
| `@tellma/core-ui` | Styled `tm-*` components — thin adapters that wire Angular `input()`/`model()` into a pattern and bind host ARIA out. The primary distribution import. |
...
```mermaid
flowchart TB
    cdk["@angular/cdk + @angular/aria"] --> prim["@tellma/core-ui-primitives<br/>(Tm*Pattern, SignalLike)"]
    prim --> ui["@tellma/core-ui<br/>(styled tm-* components)"]
...
**Conventions.** Prefix `tm-` / `Tm…` on every selector, class, token, and provider, enforced by an ESLint selector rule. Public API is signal-first (`input()`/`model()`/`output()`); components are zoneless and OnPush-equivalent. Templates are external `.html` with `@if`/`@for`/`@let`; static slots use attribute-selector `ng-content` (a documented `[tmXxx]` convention, never bare CSS-class selectors); data-bearing slots use typed `ng-template` contexts guarded by `ngTemplateContextGuard`. Config is supplied through per-feature injection tokens + `provideTm*()` functions; composition is via `hostDirectives` over inheritance; anything pluggable is an adapter.

**Theming.** Typed TS/JSON design tokens in three tiers (primitive → semantic → component). The `TmTokens` contract generates a JSON Schema; presets are validated at build against the schema and a WCAG-contrast check, so a theme that breaks contrast or references a missing token fails the build. Themes are runtime-switchable via CSS variables + `@layer`, with dark mode by selector and per-instance overrides via `dt()` + a typed passthrough; density, typography, and forced-colors are first-class. Emission needs no server rendering: base and default-theme CSS are precompiled and shipped static, and each distribution's token deltas are emitted at build time into a static stylesheet baked into its `index.html` (no runtime or server-rendered injection). **Per-distribution themes are authored mostly by agents** against the schema and via MCP `generate_theme` / `validate_theme` tools.

**Accessibility & i18n.** Accessibility is built in (CDK a11y utilities + `@angular/aria` patterns), with axe-core as a CI gate. RTL/Arabic is first-class: CDK `Directionality` auto-detection, CSS logical properties throughout, and RTL-aware keyboard navigation. The library's own strings are a single typed `TmUiStrings` object sourced through an injectable adapter, so a distribution feeds them from Transloco (scoped/lazy), `@angular/localize`, or static presets; default locale presets ship in-package. Dates, numbers, and currency format through swappable `TmDateAdapter` / `TmNumberAdapter` / `TmCurrencyAdapter` — e.g. a Hijri calendar provided by a Locale pack.

**Forms.** Every input implements `ControlValueAccessor` and exposes a signal `value = model<T>()`, so it works with reactive/template forms and Angular's Signal Forms from day one. Cross-cutting validation, messages, and error-display policy are centralized in `provideTellmaForms()`. The CVA path is the stable fallback while Signal Forms matures.

**Icons.** SVG only, wrapped behind `tm-icon` + a `TmIconRegistry` so the underlying set is swappable per distribution and sanitized (Trusted Types). The default set is Lucide; the registry keeps that an implementation detail.

**Quality & docs.** API goldens per entry point (Microsoft API Extractor + an `approve-api` CI gate) prevent silent public-API drift; every `@deprecated` is paired with an enforced `@breaking-change <version>` (the platform-wide convention). CI runs unit + harness + axe + bundle-size budget; lint is ESLint flat config + prettier + commitlint. Docs are generated from source: typed inputs + JSDoc + co-located `*.stories.ts` → API Extractor + a thin extractor → **`components.json`**, the single source of truth that feeds a Storybook showcase, `llms.txt`, the MCP server, scaffold/validate tooling, and the API goldens. The generated metadata, harnesses, uniform naming, and MCP make the library legible to coding agents by construction.
...
- **Compute:** Azure App Service is the default for distributions, the landing page, and the identity provider. Container Apps is a possible future swap.
...
- Before the swap, the distribution's migrator runs migrate + versioned seeds across the Catalog DB and every tenant DB (see [Migrations & seeding — the deploy-time migrator](#migrations--seeding--the-deploy-time-migrator)); the swap proceeds only when all tenant DBs have converged.
...
- **`@angular/aria` maturity:** it is new/experimental in Angular 22. Which aria primitives are stable enough to build `@tellma/core-ui-primitives` on at v1, versus implemented in-house and migrated later, needs a per-primitive call.
- **UI theme-builder surface:** per-distribution themes are authored mostly by agents against the token JSON Schema (and via MCP `generate_theme` / `validate_theme`). Whether a human theme-builder GUI is also built — and how much UX wraps the token contract — is open.
