...
- **UI component-library Phase 1:** the forms walking skeleton (text input, checkbox, select, plus `tm-form-field`) is specified in [`docs/specs/0002-component-library-phase1.md`](docs/specs/0002-component-library-phase1.md). It resolves several prior open items: `@angular/aria` and Signal Forms are **stable in v22** (build on both directly; no in-house aria fallback, no CVA), there is **no theme-builder GUI** (themes are CSS custom properties, authored as CSS or set at runtime), and runtime density/typography axes are deferred-but-designed-for.
